...
//                 //     'name' => $product->name,
//                 //     'price' => $product->price,
//                 //     'image_url' => ->getFirstImageUrl($item->images),
//                 //     'discount' => $product->discount,
//                 // ],
//                 'product' => $this->productFormatter->formatProduct($product),
//             ];
...
class CartService
{
    ...
    public function getCart($user): array
    {
        ...
        $this->cleanInvalidItems($user); // Очищаем недействительные элементы
        $cart->refresh();

        return $this->productFormatter->formatCartItems($cart->items);
    }
    ...
    public function addItem($user, array $data): Cart
    {
        $product = Item::find($data['product_id']);
        if (!$product) {
            Log::error('Cannot add item to cart, product not found', ['product_id' => $data['product_id']]);
    ...
    public function syncCart($user, array $items): Cart
    {
        ...
        foreach ($items as $newItem) {
            $product = Item::find($newItem['product_id']);
            if (!$product) {
                Log::warning('Skipping item in syncCart, product not found', ['product_id' => $newItem['product_id']]);
                continue;
            }
    ...
    /**
     * Очистить недействительные элементы корзины
     */
    public function cleanInvalidItems($user): Cart
    {
        $cart = Cart::where('user_id', $user->id)->first();
        if (!$cart) {
            return Cart::firstOrCreate(['user_id' => $user->id], ['items' => []]);
        }

        $items = collect($cart->items ?? [])->filter(function ($item) {
            return Item::find($item['product_id']) !== null;
        })->values()->toArray();
