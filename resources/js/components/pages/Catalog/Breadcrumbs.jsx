...
const Breadcrumbs = () => {
  ...
  const isFromSearch = queryParams.get('fromSearch') === 'true' || pathnames[0] === 'search';
  const searchQuery = queryParams.get('query') || '';
  const { filters, sortOption } = location.state || {}; // Извлекаем filters и sortOption

  console.log('Breadcrumbs: location.state', { filters, sortOption }); // Отладка
  ...
  if (pathnames[0] === 'search') {
    breadcrumbItems.push({
      ...
      path: {
        pathname: '/search',
        search: searchQuery ? `query=${encodeURIComponent(searchQuery)}` : '',
        state: { filters, sortOption }, // Передаем state
      },
      isActive: pathnames.length === 1,
    });
        ...
        <span key={index} className="breadcrumb-item">
          ...
          {item.isActive ? (
            ...
            <Link
              ...
              state={item.path.state} // Передаем state
              onClick={() => console.log('Breadcrumbs: Link clicked', { path: item.path, state: item.path.state })} // Отладка
            >
              {item.name}
            </Link>
          )}
